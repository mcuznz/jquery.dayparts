...
(function (factory) {
    ...
    $.fn.dayparts = function( options ) {
        ...
        var minutesToHour = function(hour, minute) {
            if (settings.hourPart == 'half' && minute == 1) {
                minute = 2;
            }
            var min = Math.round(minute*25/1.666);
        ...
        var hour_part = {'full': 1, 'half': 2, 'quarter': 4};

        var $table = $('<table />').addClass('dayparts table table-bordered table-condensed');
        ...
        var presetLoader = function(preset) {
            ...
            updateValue($table.find('.dayparts-span'));
        ...
        var $hour_tr = $("<tr />");
        for (var i=0; i<24; i++) {
            var $hour_td = $("<th />").addClass('cell-label hour-label text-center')
                ...
                .click(function(e){
                    ...
                    $hours = $tbody.find('span:regex(class, hour-' + $(this).data('hour') + '.*)');
                    ...
                    var currState = 0;
                    if (numActive == 7 * hour_part[settings.hourPart]) currState = 1;

                    console.log(numActive);

                    dataChange(
                        {day: 0, hour: $(this).data('hour'), state: currState},
                        {day: 6, hour: $(this).data('hour') + '.75', state: currState},
                        false);
                });
            if (i < 12) {
        ...
        for (var i=0; i<7;i++) {
            ...
            var $this_label = $("<td />").addClass('cell-label day-label')
                ...
                .click(function(e) {
                    ...
                    // See if *any* cells in this hour are state=1
                    $days = $tbody.find("td.dayparts-cell span.day-" + $(this).data('day'));
                    ...
                    var currState = 0;
                    if (numActive == 24 * hour_part[settings.hourPart]) currState = 1;

                    dataChange(
                        {day: $(this).data('day'), hour: 0, state: currState},
                        {day: $(this).data('day'), hour: 23.75, state: currState},
                        false);
                });
            ...
            for (var j=0; j<24; j++) {
                ...
                for (var k=0; k<hour_part[settings.hourPart];k++) {
                    var $minute_span = $("<span />").addClass('dayparts-span day-'+day+' hour-'+minutesToHour(j, k)).css('width', (100 / hour_part[settings.hourPart]) + '%')
                        .data('hour', minutesToHour(j, k))
                        .data('day', day)
                        .data('state', 0)
        ...
        if (value && value.length) {
            $.each(value, function(index, val) {
                ...
                if (val.day !== undefined && val.hour !== undefined) {
                    $table.find('span.day-' + val.day + '.hour-' + String(val.hour).replace('.', '\\.'))
    ...
    var toggleGrid = function(dataA, dataB, $grid, dragging) {
        ...
        var $cells = $grid.find('.dayparts-span');
